...
const AnimatedBackground = () => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            ctx.fillStyle = 'rgba(30, 64, 175, 0.5)'; // Dark blue particle color for contrast
            particles.forEach(p => {
                ctx.beginPath();
    ...
    return <canvas ref={canvasRef} id="animated-bg" className="fixed top-0 left-0 -z-10" />;
};
...
export default function TimeSeriesPage() {
    ...
    // --- UPDATED: Function to handle API call ---
    const handleProcessImages = async () => { // Step 1: Make the function async
        if (!file1 || !file2) {
        ...
        // Step 2: Create a FormData object to send files
        ...
        try {
            // Step 3: Use fetch to send the data to your API endpoint
            ...
            // Step 5: Parse the JSON response from the API
            const result = await response.json();
            
            // Step 6: Update the state with the results from the API
            setAnalysisResult(result);

        } catch (error: any) {
            // Step 7: Handle any errors that occurred during the fetch
            console.error("Processing error:", error);
            alert(error.message); // Show a more user-friendly error message
        } finally {
            // Step 8: Ensure the processing state is set to false, even if there was an error
