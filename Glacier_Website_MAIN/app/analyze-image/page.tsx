...
export default function AnalyzeImagePage() {
    ...
    const handleProcessImage = async () => { // Make the function async
        if (!file) {
        ...
        // This FormData object is what you will send to your real AI model API
        const formData = new FormData();
        formData.append('image', file);

        try {
            // This fetch call sends the image to your new API route
            const response = await fetch('/api/analyze-single-image', {
                method: 'POST',
                body: formData,
            });
        ...
        <div className="min-h-screen bg-blue-50 text-slate-800 font-sans">
            ...
            <main className="pt-32 pb-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div id="upload-tool" className="glass-container-blue rounded-lg p-8 mb-12 max-w-2xl mx-auto opacity-0">
                        <div {...getRootProps()} className={`file-input-label-blue transition-colors p-8 ${isDragActive ? 'bg-blue-100' : 'hover:bg-blue-50'}`}>
                            <input {...getInputProps()} />
                        ...
                        <div className="mt-8 text-center">
                            <Button onClick={handleProcessImage} disabled={!file || isProcessing} className="btn-analyze-blue font-bold py-3 px-8 rounded-md text-lg h-auto disabled:bg-gray-400 disabled:scale-100">
                    ...
                    {analysisResult && (
                        <div id="results-section" className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <Card className="glass-container-blue">
                                    <CardHeader><CardTitle className="text-center">Original Image</CardTitle></CardHeader>
                                    <CardContent>
                                        <Image onClick={() => setLightboxImage(preview)} width={600} height={400} src={preview || "https://placehold.co/600x400/e2e8f0/475569?text=Original+Image"} alt="Original uploaded" className="rounded-md w-full cursor-zoom-in transition-transform hover:scale-[1.02]" />
                                    </CardContent>
                                </Card>
                                 <Card className="glass-container-blue">
                                    <CardHeader><CardTitle className="text-center">Processed Image (Masked)</CardTitle></CardHeader>
                                    <CardContent>
                                         <Image onClick={() => setLightboxImage(analysisResult.processedImageUrl)} width={600} height={400} src={analysisResult.processedImageUrl} alt="Processed lakes" className="rounded-md w-full cursor-zoom-in transition-transform hover:scale-[1.02]" />
                                    </CardContent>
                                </Card>
                            </div>

                            <Card className="glass-container-blue p-6 max-w-md mx-auto">
                                <CardHeader><CardTitle className="text-center">Key Statistics</CardTitle></CardHeader>
                                <CardContent className="space-y-3 text-lg text-center">
                                    ...
                                    <p>Total Lake Area: <span className="font-bold text-blue-600">{analysisResult.totalArea} km²</span></p>
                                    
                                    <div className="pt-4 border-t border-blue-200/50">
                                        <button onClick={() => setIsDetailsOpen(!isDetailsOpen)} className="text-sm text-blue-600 hover:underline focus:outline-none w-full flex justify-center items-center">
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center" onClick={onClose}>
            <div className="absolute top-4 right-4 flex gap-4">
                <Button onClick={onClose} variant="outline" size="icon" className="bg-white/20 border-white/50 hover:bg-white/30 text-white"><X /></Button>
            </div>
