...
import sikkimShape from "@/public/sikkim_shape.geojson";
...
function GISMap() {
  ...
  useEffect(() => {
    if (lakeMaskLayerRef.current) {
      if (showLakeMask) mapInstance.current?.addLayer(lakeMaskLayerRef.current);
      else mapInstance.current?.removeLayer(lakeMaskLayerRef.current);
    }
  }, [showLakeMask]);
